...
class AdminWindow(BoxLayout):
    def __init__(self, **kwargs):
        ...
        #Display Products
        product_scrn = self.ids.scrn_products_contents
    ...
    def add_product(self, code,name,weight,stock,sold,order,purchase):
        ...
        content.clear_widgets()
        if code == '' or name == '' or weight == '' or stock == '':
            self.notify.add_widget(Label(text='[color=#FF0000][b]ID, Name, Weight, and Stock Required![/b][/color]',markup=True))
            self.notify.open()
            Clock.schedule_once(self.killswitch,1)
        else:
            ...
            stockstable = DataTable(table=prodz)
    ...
    def update_product(self, code,name,weight,stock,sold,order,purchase):
        if code == '':
            self.notify.add_widget(Label(text='[color=#FF0000][b]Product ID Required![/b][/color]',markup=True))
            self.notify.open()
            Clock.schedule_once(self.killswitch,1)
        else:
            target_code = self.products.find_one({'product_code':code})
            if target_code == None:
                self.notify.add_widget(Label(text='[color=#FF0000][b]Invalid Product ID![/b][/color]',markup=True))
                self.notify.open()
                Clock.schedule_once(self.killswitch,1)
            else:
                if name == '':
    ...
    def remove_user_fields(self):
        ...
        crud_submit = Button(text='Update',size_hint_x=None,width=100,on_release=lambda x: self.remove_user(crud_user.text))
        target.add_widget(crud_user)
        target.add_widget(crud_submit)
    ...
    def remove_product_fields(self):
        ...
        crud_submit = Button(text='Update',size_hint_x=None,width=100,on_release=lambda x: self.remove_product(crud_code.text))
        target.add_widget(crud_code)
        target.add_widget(crud_submit)
    ...
    def view_stats(self):
        ...
        df = pd.read_csv('product_purchases.csv')
        purchases = []
        dates = []
        count = 0
